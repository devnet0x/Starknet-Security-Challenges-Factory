...
import challengeCode from '../assets/challenge9.cairo'
import '../App.css';
...
function Status(){
    ...
    return(
        <span>
        {data && !parseInt(data[0].toString())?<Challenge9Deploy />:<span>Already Resolved</span>}
        </span>
    ) 
}
...
function Challenge9Deploy() {
    ...
    return (
      <>
        ...
        {data && (data.status=="ACCEPTED_ON_L2"||data.status=="ACCEPTED_ON_L1") && data.events?<div> <Challenge9Check /> </div>:<div></div>}
       </>
    )
}

function Challenge9Check() {
    const [hash, setHash] = useState(undefined)
    const { data, loading, error } = useTransactionReceipt({ hash, watch: true })
    ...
    <div className="App" class='flex-table row' role='rowgroup'>
      ...
      <div class='flex-row-wide' role='cell'>
        <StarknetConfig connectors={connectors}>
          <p><font size="+2"><b>FAL1OUT</b></font></p>
