...
import challengeCode from '../assets/challenge10.cairo'
import '../App.css';
...
function Status(){
    ...
    return(
        <span>
        {data && !parseInt(data[0].toString())?<Challenge10Deploy />:<span>Already Resolved</span>}
        </span>
    ) 
}
...
function Challenge10Deploy() {
    ...
    return (
      <>
        ...
        {data && (data.status=="ACCEPTED_ON_L2"||data.status=="ACCEPTED_ON_L1") && data.events?<div> <Challenge10Check /> </div>:<div></div>}
       </>
    )
}

function Challenge10Check() {
    const [hash, setHash] = useState(undefined)
    const { data, loading, error } = useTransactionReceipt({ hash, watch: true })
    ...
    <div className="App" class='flex-table row' role='rowgroup'>
      ...
      <ToggleSwitch id={chkID} checked={lang} optionLabels={textOptions} small={true} onChange={checked => setLang(checked)} />
        ...
        <StarknetConfig connectors={connectors}>
          <p><font size="+2"><b>COIN FLIP</b></font></p>
  ...
  return (
    <div className="App" class='flex-table row' role='rowgroup'>
      ...
      <ToggleSwitch id={chkID} checked={lang} optionLabels={textOptions} small={true} onChange={checked => setLang(checked)} />
        ...
        <StarknetConfig connectors={connectors}>
          <p><font size="+2"><b>COIN FLIP</b></font></p>
          Este es un juego de lanzamiento de monedas en el que debes construir tu racha ganadora adivinando el resultado del lanzamiento.
