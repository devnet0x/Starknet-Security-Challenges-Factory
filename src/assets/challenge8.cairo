...
mod Deployer {
    ...
    fn constructor(ref self: ContractState) {
        ...
        let calldata = serialize_calldata(@deployer_address, TOKEN_100.try_into().unwrap());

        // Deploy ERC20 ISEC and mint 100 ISEC
        let isec_class_hash: ClassHash = starknet::class_hash_const::<
            0x963950860a14c82730491fb9303b9cd76a82dfb083e28ce95c12e064954f36 // to modify?
        >();
        ...
        let iset_class_hash: ClassHash = starknet::class_hash_const::<
            0x03699b10f3fca2869c6684672cdb29721b3bbcc9123f10edf4813112a5b5b82e // to modify?
        >();

        let (iset, _) = deploy_syscall(iset_class_hash, current_salt, calldata.span(), false,)
            .unwrap();
        self.iset_address.write(iset);
        self.salt.write(current_salt + 1);

        // Deploy DEX
        let calldata2 = serialize_dex_calldata(@self.get_isec_address(), @self.get_iset_address());

        let dex_class_hash: ClassHash = starknet::class_hash_const::<
            0x00dcc8752dbdbe0d2ad3771a9d4a438a7d8ed19294bd2bec923f0dc282ba78a0 // to modify?
        >();
        ...
        IERC20Dispatcher { contract_address: self.get_iset_address() }
            .approve(self.get_dex_address(), TOKEN_10);
    ...
    impl Main of IMain {
        ...
        fn is_complete(self: @ContractState) -> bool {
            ...
            let dex_iset_balance = IERC20Dispatcher { contract_address: self.get_iset_address() }
                .balance_of(self.get_dex_address());
    ...
    // Free functions
    fn serialize_calldata(caller_address: @ContractAddress, token_amount: u32) -> Array<felt252> {
